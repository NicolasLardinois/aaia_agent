  ...
  **Ansatz:** `side` (long/short) je Position in `portfolio.json`; short-bewusste P&L (invertiert) + Netto-Exposure; daraus `current_position` (none/long/short) ableiten; Reconciliation (beide Linsen feuern).
  ...
  - [ ] **Risiko-Kennzahlen verfeinern: Beta-/Korrelations-bereinigtes Netto-Exposure + ETF-Look-Through** *(Befund 2026-06-20 aus PR#7-Review, fachliche Folge von 3a — User-Einwand).*
    ...
    **Status & verbindliche Sequenz (2026-06-20) — NICHT vergessen:** in drei Schritte zerlegt:
    - **F4a — `net_beta` pro Region + `returns_provider`/Vola produktiv** → 🔵 **in Arbeit** (Branch `feat/risk-net-beta-vola`; Spec `docs/superpowers/specs/2026-06-20-net-beta-vola-design.md`). `net_beta = Σ(signed_value·β)` je Region als **$-Hedge-Notional** (Beta via `get_info`, fehlend→1,0); Vola wird live (heute konstant 0).
    ...
    **Gesamt-Reihenfolge des Shorts-Programms (ab hier):** **F4a** → **F4b (ETF-Look-Through)** → **SHORT+-Aktivierung** (Short-Engine nutzt Einstand/P&L, durch 3a freigeschaltet) → **3b Track-B-Hedge** (dimensioniert auf `net_beta`, pro Region der richtige Index) → **Block #4 Short-Backtest**. *(Roadmap zentral hier im Logbuch; `short.md` = nur Design.)*
