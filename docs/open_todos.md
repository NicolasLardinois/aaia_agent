...
- [ ] **Money-Supply velocity-Modifikator** (`agents/market_cockpit/macro/money_supply_agent.py`) *(Minor)*.
  ...
  **Ansatz:** Velocity-Trend (USA, `money_velocity`/M2V) ableiten — aktuelle vs. vorherige Umlaufgeschwindigkeit via persistenter `DatedHistoryPort` (append je Lauf, `value_on_or_before` der Vorperiode → `"falling"`/`"rising"`) und als String an `_signal` geben (EU/CH haben keine Velocity → bleiben `None`).
  **Abhängigkeit (2026-06-25):** baut auf der `history`-Verdrahtung aus PR #76 (Yield-Curve-Bull-Steepening) auf — derselbe `DatedHistoryPort` muss vom `TopDownOrchestrator` zusätzlich durch den **MacroChiefAgent** zum `MoneySupplyAgent` gefädelt werden. **Erst nach Merge von #76** umsetzen, sonst kollidiert die zweite `TopDownOrchestrator.__init__(history=…)`-Erweiterung mit #76. *(Bewusst nicht als kollidierender Parallel-PR gebaut.)*
