...
- [x] **Teil-Projekt A — Onboarding + Erklär-/Metrik-Fundament** (Branch `feat/frontend-onboarding`, Plan `docs/superpowers/plans/2026-06-25-frontend-onboarding-fundament.md`). Lösung: neue **Willkommen-Seite** (`/willkommen`) erklärt AAIA + jeden der 5 Bereiche („wo finde ich was?") + den Analyse-Ablauf (Top-Down→Bottom-Up→Urteil) + den Demo-Daten-Hinweis; **Erst-Besuch-Routing** (onboarding-bewusster Index-Redirect via `useOnboarding`-Flag in `localStorage`), dauerhaft über **„?" in der Topbar** + **Sidebar-Eintrag**. Dazu der wiederverwendbare **Baukasten** für B/C: `InfoTip` (barrierearmer Fachbegriff-Tooltip), `lib/glossary` (pure Begriff→Erklärung), `SectionCard`, `MetricRow`/`MetricCard` (n.v.-fest, UNAVAILABLE ≠ 0), `welcomeContent` (eine Quelle). TDD durchgängig; volle Frontend-Suite **469 grün** (88 Dateien), `npm run build` grün. **PR offen** — Merge-Vermerk folgt nach 2. Blick.
  - **Teil-Projekt B — Kennzahlen sichtbar machen** (eigene Spec/Plan, **Deep-Dive zuerst**): alle Fundamental-/Bewertungs-/Qualitäts-Metriken pro Titel über `MetricRow`/`MetricCard` + `InfoTip`-Erklärungen; danach Cockpit-Drilldowns. Demo, wo Backend/Serializer noch nicht liefert (Tausch-Naht). Optional eigene **Glossar-Seite** (nutzt `lib/glossary`).
    - [x] **Slice B1 — Deep-Dive Aktienkennzahlen** (Branch `feat/frontend-deepdive-metrics`, gestapelt auf A; Spec `docs/superpowers/specs/2026-06-25-frontend-deepdive-kennzahlen-design.md`). Lösung: `EquityFundamentalsDTO` (Forward-KGV, Shiller-CAPE, PEG, EV/Umsatz, KBV, KUV, P/FCF, Dividendenrendite, WACC, Umsatz-CAGR, Verschuldungsgrad) optional am `EquityBlock` + Demo (AAPL, 1× `null` → „n.v."-Pfad); `lib/glossary` um alle Kennzahlen erweitert (fachliche Definitionen); `EquityTabs` auf den A-Baukasten (`MetricRow`/`SectionCard`/`InfoTip`) umgebaut — voller Katalog gruppiert (Bewertung / Profitabilität & Bilanz), Erklär-Tooltips, „n.v." statt 0. Kein Backend-Change. Frontend-Suite **474 grün**, `npm run build` grün. **PR offen.**
  ...
  - **Teil-Projekt C — UX-Politur (Trading-Plattform-Feel)** (querschnitt): aufbauend auf dem Baukasten aus A.
    - [x] **Slice C1 — Designsprache „Instrument-Deck"** (Branch `feat/frontend-design-refresh`, gestapelt auf B1; via frontend-design-Skill, kein separates Spec — Designentscheidungen im PR). Lösung: **Token-Schicht** (RGB-Kanäle in `index.css` → Tailwind-Opazitäten; Light/Dark), **ein** Marken-Akzent Kobalt-Indigo (klar getrennt von den Finanz-Signalen Grün/Rot), **drei Schriftrollen** (Space Grotesk Display / IBM Plex Sans Text / IBM Plex Mono Kennzahlen). **Signatur:** Kennzahlen als Mono-Ablesungen (`MetricRow`/`MetricCard`, tabellarisch) + Regime-„Horizont"-Band (Cockpit-Referenz). Umgesetzt: Shell (Sidebar-Wortmarke, Kommando-Topbar, Token-Hintergrund), Kern-Komponenten (`SectionCard` Eyebrow, `MetricRow`/`MetricCard` Mono, `SignalBadge` Signal-Punkt, `DemoBadge`, `InfoTip`), Willkommen-Seite als Schauseite (Hero + Horizont). DOM/Texte stabil → keine Test-Brüche; **474 grün**, Build grün. **PR offen.**
